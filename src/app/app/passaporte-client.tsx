...
export default function PassaporteClient({ perfil, usedCount, partnerCount }: Props) {
  const nome = perfil?.nome ?? "Visitante";
  const codigo = perfil?.codigo ?? "AURA-----";
  ...
  return (
    <div className="px-5 py-6 max-w-md mx-auto">
      ...
      >
        ...
        <div className="flex items-start justify-between mb-6 relative z-10">
          <div>
            ...
            <h2 className="font-serif text-xl leading-tight" style={{ color: "var(--creme)" }}>{nome}</h2>
          </div>
          <div className="w-12 h-12 rounded-full border-2 flex items-center justify-center text-xl flex-shrink-0"
            style={{ borderColor: "var(--verde-salvia)", backgroundColor: "rgba(255,255,255,0.1)" }}>
            🌿
          </div>
        </div>
        ...
        <div className="relative z-10 mb-5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs opacity-60" style={{ color: "var(--verde-nevoa)" }}>até {validade}</span>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-end justify-between">
          <div>
            ...
            <p className="font-mono text-sm font-medium tracking-wider" style={{ color: "var(--creme)" }}>{codigo}</p>
            <p className="text-xs mt-0.5 opacity-50" style={{ color: "var(--verde-nevoa)" }}>{tipo}</p>
          </div>
          <div className="rounded-xl p-2" style={{ backgroundColor: "var(--papel)" }}>
            <QRCode value={codigo} size={72} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="rounded-xl p-4" style={{ backgroundColor: "var(--creme)" }}>
          <p className="text-2xl font-serif mb-1" style={{ color: "var(--verde-aura)" }}>{partnerCount}</p>
